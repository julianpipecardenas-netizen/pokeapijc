...
app.post("/pokemones", (req, res) => {
  ...
  pool.query(sql, [nombre, imagen_frontal, imagen_posterior, imagen_shiny, altura, peso, tipos], (err, result) => {
    if (err) return res.json(err);
...
app.put("/pokemones/:id", (req, res) => {
  ...
  pool.query(sql, [nombre, imagen_frontal, imagen_posterior, imagen_shiny, altura, peso, tipos, id], (err, result) => {
    if (err) return res.json(err);
...
app.delete("/pokemones/:id", (req, res) => {
  ...
  pool.query("DELETE FROM pokemones WHERE id = ?", [id], (err, result) => {
    if (err) return res.json(err);
